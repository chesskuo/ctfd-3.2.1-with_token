...
def register():
    ...
    if request.method == "POST":
        secret_token = request.form.get("secret_token", "").strip()
        if __import__("CTFd.constants.config", fromlist=("Configs")).Configs.secret_token_enable and secret_token != __import__("os").getenv("SECRET_TOKEN"):
            return "Wrong Token :("
